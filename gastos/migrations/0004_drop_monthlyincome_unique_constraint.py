...
def drop_monthly_income_unique_constraint(apps, schema_editor):
    ...
    matching_names = [
        ...
        if details.get('unique') and set(details.get('columns') or []) == columns
    ]
    ...
    if connection.vendor == 'postgresql':
        for name in matching_names:
            schema_editor.execute(f'ALTER TABLE {quote(table)} DROP CONSTRAINT IF EXISTS {quote(name)}')
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('gastos', '0003_remove_monthlyincome_unique_db_constraint'),
    ]

    operations = [
        migrations.RunPython(drop_monthly_income_unique_constraint, migrations.RunPython.noop),
    ]
